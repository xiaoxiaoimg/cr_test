...
- 在snake.py中0，-10可以声明为常量，违反了CR点13。
- 在snake.py中使用了raise Exception来处理边界碰撞，而不是使用异常类，这违反了CR点25。
- 在game_utils.py中的load_image函数中使用了sys.exit()来退出程序，这违反了CR点28。
- 在game_utils.py中使用 print 函数记录日志，违反了CR点33。
- 在main.py中screen_height常量，使用全大写字母，并用下划线分隔，违反了CR点8。
- 在main.py中tick(10)，10可以声明为常量，违反了CR点13。
...
- ui.py中公共有12处，使用 print 函数，违反了CR点33。
...
- ID 11：在 game_board.py 中，避免使用except:来捕获所有异常，应该指定异常类型。
...
- ID 26：在 transaction.py 中使用 eval() 执行字符串作为代码,2个地方。
